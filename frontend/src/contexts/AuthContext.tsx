...
const normalizeUser = (user: any, fallbackRole?: "citizen" | "admin"): User => ({
  id: user.id || user._id,
  _id: user._id,
  email: user.email,
  fullName: user.fullName || "Guest",
  role: user.role || fallbackRole || "citizen",
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const storedRole = (localStorage.getItem("auth_role") ||
        parsedStoredUser?.role) as "citizen" | "admin" | null;
      const storedUserId = localStorage.getItem("auth_user_id") || parsedStoredUser?.id;
      ...
      if (response.ok) {
        const freshUser = normalizeUser(result, storedRole);
        setUser(freshUser);
  ...
  const login = async (
    ...
    try {
      ...
      const authUser = normalizeUser(result.user, role);

      setToken(result.token);
      setUser(authUser);

      persistAuth(result.token, authUser);
